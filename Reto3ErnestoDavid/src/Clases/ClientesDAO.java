...
public class ClientesDAO {
	...
	/**
	 * Funcion que recibe el un cliente y con su codigo busca los datos de ese cliente
	 * @param cli Cliente
	 * @return Cliente
	 */
	public static Clientes buscarCliente(Clientes cli) {
		...
		try (Connection con = Conexion.abreConexion()) {
			...
			if (rs.next()) {
				clienteNuevo = (new Clientes(rs.getInt("idcliente"), rs.getString("nombre"), rs.getString("direccion"),
						rs.getInt("codigo")));
				return clienteNuevo;
			} else {
				return null;
	...
	public static void nuevoCliente(Clientes cli) {
		try (Connection con = Conexion.abreConexion()) {
			PreparedStatement stmt = con
					.prepareStatement("insert into clientes(nombre, direccion, codigo)\r\n" + "values(?,?,?);");
			stmt.setString(1, cli.getNombre());
			stmt.setString(2, cli.getDireccion());
			stmt.setInt(3, cli.getCodigo());
